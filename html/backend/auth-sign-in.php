...
if (isset($_POST['signIn'])) {
   ...
   $check_query = "SELECT SID, SPASS FROM store WHERE SEMAIL=? LIMIT 1";
   ...
   if ($check_stmt->num_rows > 0) {
      $check_stmt->bind_result($storedSID, $storedPassword);
      $check_stmt->fetch();

      // Verify the password
      if (password_verify($password, $storedPassword)) {
         $_SESSION['login_user'] = $email;
         header("location: index1.php"); // Redirect to the dashboard or home page
         exit();
      } else {
